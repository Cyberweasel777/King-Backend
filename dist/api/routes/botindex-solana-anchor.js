...
const client_1 = require("../../services/solana/client");
const router = (0, express_1.Router)();
const solanaClient = new client_1.SolanaClient();
const DATA_DIR = process.env.DATA_DIR || path_1.default.join(process.cwd(), 'data');
const LEDGER_FILE = path_1.default.join(DATA_DIR, 'solana-anchors.json');
...
function normalizeLedgerEntry(value) {
    ...
    const txSignature = typeof row.txSignature === 'string' ? (0, client_1.normalizeTxSignature)(row.txSignature) : null;
    const slot = typeof row.slot === 'number' && Number.isInteger(row.slot) && row.slot >= 0
        ? row.slot
        : null;
...
function loadLedgerFromDisk() {
    try {
        ...
        logger_1.default.info({ anchors: anchorLedger.anchors.length }, 'Solana anchor ledger loaded');
    }
    catch (err) {
        logger_1.default.warn({ err }, 'Failed to load Solana anchor ledger, starting fresh');
    }
}
function flushLedgerToDisk() {
    try {
        ...
        logger_1.default.warn({ err }, 'Failed to flush Solana anchor ledger');
    }
}
...
loadLedgerFromDisk();
router.post('/solana/aar/anchor', (0, x402Gate_1.createX402Gate)({ price: '$0.01', description: 'Anchor signed AAR proof to Solana memo registry (0.01 USDC)' }), async (req, res) => {
    ...
    try {
        ...
        logger_1.default.error({ err: error }, 'Failed to anchor AAR proof to Solana');
        res.status(500).json({
...
router.get('/solana/aar/verify/:proofHash', async (req, res) => {
    ...
    try {
        ...
        logger_1.default.warn({ err: error, proofHash }, 'Solana chain verification query failed');
    }
    res.json({
        verified: false,
        anchorMeta: null,
...
router.get('/solana/aar/registry', (0, x402Gate_1.createX402Gate)({ price: '$0.01', description: 'Paginated Solana AAR anchor registry (0.01 USDC)' }), async (req, res) => {
    ...
    try {
        ...
        if (includeChain) {
            try {
                ...
                logger_1.default.warn({ err: error }, 'Failed to query Solana anchors, returning local registry');
            }
        }
        ...
        logger_1.default.error({ err: error }, 'Failed to fetch Solana anchor registry');
        res.status(500).json({
...
//# sourceMappingURL=botindex-solana-anchor.js.map