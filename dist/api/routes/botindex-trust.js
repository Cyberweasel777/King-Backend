...
const aar_service_1 = require("../../services/botindex/trust/aar-service");
const scc_service_1 = require("../../services/botindex/trust/scc-service");
...
function readSessionIndex(body) {
...
function parseSCCCertificate(value) {
    const body = asRecord(value);
    if (!body)
        return null;
...
router.post('/trust/aar/sign', (0, x402Gate_1.createX402Gate)({ price: '$0.001', description: 'Sign an Agent Action Receipt' }), async (req, res) => {
    try {
        ...
        const receipt = (0, aar_service_1.signReceipt)({
            agent,
            principal,
            action,
            scope,
        ...
        logger_1.default.error({ err: error }, 'Failed to sign BotIndex AAR');
        res.status(500).json({
...
router.post('/trust/aar/verify', (req, res) => {
    const body = asRecord(req.body);
    if (!body) {
        res.status(400).json({
            error: 'invalid_payload',
    ...
    const publicKey = typeof body.publicKey === 'string' && body.publicKey.trim() ? body.publicKey.trim() : undefined;
    const result = (0, aar_service_1.verifyReceipt)(receipt, publicKey);
    res.json(result);
});
router.post('/trust/scc/anchor', (0, x402Gate_1.createX402Gate)({ price: '$0.01', description: 'Anchor a Session Continuity Certificate' }), async (req, res) => {
    try {
        const body = asRecord(req.body);
        if (!body) {
            res.status(400).json({
        ...
        if (!agentId || sessionIndex === null || !memoryRoot || !capabilityHash || !stateHash) {
            res.status(400).json({
                error: 'invalid_payload',
                message: 'Fields agentId, sessionIndex, memoryRoot, capabilityHash, and stateHash are required',
            });
            return;
        }
        const anchored = (0, scc_service_1.anchorSCC)({
            agentId,
            sessionIndex,
        ...
        logger_1.default.error({ err: error }, 'Failed to anchor SCC certificate');
        res.status(500).json({
...
router.get('/trust/scc/verify/:anchorHash', (req, res) => {
    ...
    const verification = (0, scc_service_1.verifyAnchor)(anchorHash);
...
router.post('/trust/scc/verify-chain', (req, res) => {
    const body = asRecord(req.body);
    ...
    const result = (0, scc_service_1.verifySCCChain)(certificates);
    res.json(result);
});
router.get('/trust/verify', (_req, res) => {
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.send(trustPortalHtml());
});
exports.default = router;
//# sourceMappingURL=botindex-trust.js.map