...
function mountBotindexX402TestRoute() {
    if (x402RouteMounted)
        return;
    router.use(['/x402/correlation-leaders', '/v1/signals'], apiKeyAuth_1.optionalApiKey, (req, res, next) => {
        if (!req.apiKeyAuth) {
            next();
            return;
        }
    ...
    router.use('/x402', x402_test_1.default);
    router.use('/v1', x402_premium_1.default);
    router.use('/v1', botindex_sports_1.default);
    router.use('/v1', botindex_crypto_1.default);
    router.use('/v1', botindex_genesis_1.default);
    router.use('/v1', botindex_commerce_1.default);
    x402RouteMounted = true;
}
const manualSignals = new Map();
...
router.get('/signals', async (req, res) => {
    ...
    try {
        ...
        const matrix = (0, matrix_1.generateCorrelationMatrix)(priceSeries, '24h');
        const top = dedupeCorrelationPairs((0, matrix_1.getTopCorrelatedPairs)(matrix, Math.max(limit * 3, 10), true)).slice(0, limit);
        const generated = top.map((p, i) => ({
            id: `corr-${Date.now()}-${i}`,
        ...
        const isPaid = req.__freeTrialAuthenticated || req.apiKeyAuth?.plan === 'pro';
...
router.use(async (req, res, next) => {
    ...
    if (isPairCorrelation) {
        return (0, payments_1.withFreeLimit)({ perDay: 5, key: payments_1.getFreeLimitKey })(req, res, next);
    }
    return (0, payments_1.withSubscriptionHttp)('botindex', 'pro')(req, res, next);
}, correlation_routes_1.default);
