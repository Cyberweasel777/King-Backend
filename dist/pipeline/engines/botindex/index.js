...
exports.runBotIndexPipeline = runBotIndexPipeline;
const logger_1 = require("../../../utils/logger");
// Data collection functions
async function collectBotMetrics() {
    logger_1.logger.info('Collecting bot metrics from various sources');
    const metrics = [];
...
async function fetchPlatformMetrics(platform, category) {
    logger_1.logger.debug({ platform, category }, 'Fetching platform metrics');
...
function calculateBotScore(metrics) {
    let score = 0;
    // Performance score (30%)
    const perfScore = ((Math.max(0, 100 - metrics.responseTimeMs / 10)) * 0.1 +
...
function categorizeBots(metrics) {
    logger_1.logger.info('Categorizing and ranking bots');
    // Group by category
...
function detectTrends(metrics) {
    logger_1.logger.info('Detecting bot trends');
    const rising = [];
    const falling = [];
    const newEntries = [];
    for (const bot of metrics) {
...
// Storage functions
async function storeBotMetrics(metrics) {
    logger_1.logger.info(`Storing ${metrics.length} bot metrics`);
    ...
    for (let i = 0; i < metrics.length; i += batchSize) {
        ...
        logger_1.logger.debug(`Storing batch ${i / batchSize + 1} (${batch.length} bots)`);
        // await db.insert(botMetrics).values(batch);
    }
}
async function storeRankings(rankings) {
    logger_1.logger.info(`Storing rankings for ${rankings.length} categories`);
...
async function warmCache(rankings) {
    logger_1.logger.info('Warming cache for popular rankings');
    ...
    for (const category of popularCategories) {
        ...
        if (ranking) {
            ...
            logger_1.logger.debug({ category, topBot: ranking.topPerformer?.name }, 'Cache warmed');
        }
    }
}
// Main pipeline function
async function runBotIndexPipeline() {
    logger_1.logger.info('Running BotIndex pipeline');
    const startTime = Date.now();
    try {
        // 1. Collect metrics from all sources
        const metrics = await collectBotMetrics();
        if (metrics.length === 0) {
            logger_1.logger.warn('No bot metrics collected');
            return;
        }
        logger_1.logger.info(`Collected metrics for ${metrics.length} bots`);
        // 2. Calculate scores
        for (const bot of metrics) {
        ...
        logger_1.logger.error({ error }, 'BotIndex pipeline failed');
        throw error;
    }
}
// Alternative export name for consistency
exports.runPipeline = runBotIndexPipeline;
