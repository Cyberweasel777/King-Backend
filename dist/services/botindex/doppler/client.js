...
exports.dopplerClient = exports.DopplerClient = void 0;
...
let viemRuntimeCache = null;
function loadViemRuntime() {
    if (viemRuntimeCache) {
        return viemRuntimeCache;
    }
    const viemModule = require('viem');
    const chainModule = require('viem/chains');
...
`;
const DEFAULT_ASSETS_QUERY = (0, graphql_request_1.gql) `
...
`;
const TOKENS_QUERY = (0, graphql_request_1.gql) `
...
`;
const ASSET_DETAILS_QUERY = (0, graphql_request_1.gql) `
...
`;
const TOKEN_DETAILS_QUERY = (0, graphql_request_1.gql) `
...
class DopplerClient {
    ...
    async getRecentLaunches(hours, limit) {
    ...
    async getAssetDetails(address) {
        ...
        try {
            ...
            logger_1.default.warn({ err: error, address: normalizedAddress }, 'Doppler indexer asset details query failed, using Base RPC fallback');
        }
        const fallbackSeed = this.knownAssets.get(normalizedAddress);
        const fromRpc = await this.fetchAssetFromRpc(normalizedAddress, fallbackSeed);
        this.rememberAssets([fromRpc]);
        this.setCache(cacheKey, fromRpc);
        return fromRpc;
    }
    async getTrendingAssets(limit) {
        ...
        try {
            ...
            logger_1.default.warn({ err: error }, 'Doppler indexer trending query failed, falling back to Base RPC');
    ...
    mapAssetRow(assetRow, tokenRow, source) {
    ...
    async fetchIndexerAssets(limit) {
        const boundedLimit = clampLimit(limit, 50);
        let assetRows = [];
        try {
            const recentResponse = await this.indexerClient.request(RECENT_ASSETS_QUERY, {
                limit: boundedLimit,
            });
            assetRows = Array.isArray(recentResponse.assets?.items) ? recentResponse.assets.items : [];
        }
        catch (error) {
            logger_1.default.debug({ err: error }, 'Doppler indexer does not support createdAt ordering, retrying with default ordering');
            const defaultResponse = await this.indexerClient.request(DEFAULT_ASSETS_QUERY, {
                limit: boundedLimit,
            });
            assetRows = Array.isArray(defaultResponse.assets?.items) ? defaultResponse.assets.items : [];
        }
        const tokenResponse = await this.indexerClient.request(TOKENS_QUERY, {
            limit: boundedLimit,
        });
    ...
    async fetchIndexerAssetDetails(address) {
        const [assetResponse, tokenResponse] = await Promise.all([
            this.indexerClient.request(ASSET_DETAILS_QUERY, {
                address,
            }),
            this.indexerClient.request(TOKEN_DETAILS_QUERY, {
                address,
            }),
        ]);
        ...
        const integratorLaunchCount = mapped.integrator
            ? Array.from(this.knownAssets.values()).filter((asset) => asset.integrator && normalizeAddressLower(asset.integrator) === normalizeAddressLower(mapped.integrator || '')).length
            : 0;
        return {
            ...mapped,
    ...
    getFallbackAddresses(maxCount) {
        ...
        return merged.filter((candidate) => this.viem.isAddress(candidate));
    }
    async getRecentLaunchesFromRpc(hours, limit) {
        const fallbackAddresses = this.getFallbackAddresses(Math.max(limit * 5, 40));
        if (fallbackAddresses.length === 0) {
            logger_1.default.warn('No fallback Doppler addresses available for recent launches RPC fallback');
            return [];
        }
        const fetched = await Promise.allSettled(fallbackAddresses.map((address) => this.fetchAssetFromRpc(address, this.knownAssets.get(normalizeAddressLower(address)))));
    ...
    async getTrendingFromRpc(limit) {
        const fallbackAddresses = this.getFallbackAddresses(Math.max(limit * 5, 40));
        if (fallbackAddresses.length === 0) {
            logger_1.default.warn('No fallback Doppler addresses available for trending RPC fallback');
            return [];
        }
        const fetched = await Promise.allSettled(fallbackAddresses.map((address) => this.fetchAssetFromRpc(address, this.knownAssets.get(normalizeAddressLower(address)))));
    ...
    async inferCreatedAtFromLogs(address, fallbackTimestamp) {
        try {
            ...
            logger_1.default.debug({ err: error, address }, 'Failed to infer token creation timestamp from transfer logs');
            return fallbackTimestamp;
        }
    }
    async fetchAssetFromRpc(address, seed) {
...
exports.DopplerClient = DopplerClient;
exports.dopplerClient = new DopplerClient();
