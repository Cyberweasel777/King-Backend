...
exports.fetchPriceSeries = fetchPriceSeries;
exports.fetchMultiplePriceSeries = fetchMultiplePriceSeries;
 ...
 */
async function resolveTopPoolFromDexscreener(chain, tokenAddress) {
    try {
        ...
        const pairs = data.pairs
            .filter((p) => !p.chainId || String(p.chainId).toLowerCase() === String(chain).toLowerCase());
 ...
 */
async function fetchPriceSeries(token, window = '24h', source = 'fallback') {
 ...
 */
async function fetchFromDEXScreener(token, _window) {
    try {
 ...
 * @returns Price series or null
 */
async function fetchFromGeckoTerminal(token, window) {
    try {
        ...
        for (const point of ohlcvList) {
            ...
            ohlcv.push({
                ...
                open: parseFloat(String(point[1])),
                high: parseFloat(String(point[2])),
                low: parseFloat(String(point[3])),
                close: parseFloat(String(point[4])),
                volume: parseFloat(String(point[5]))
            });
        }
        return {
            token: address,
 ...
 */
async function fetchMultiplePriceSeries(tokens, window = '24h') {
...
async function getAggregatedPrice(token) {
    try {
        ...
        await rateLimit();
        const dexResponse = await fetch(`${DEXSCREENER_API}/dex/tokens/${address}`, {
            headers: {
                'Accept': 'application/json',
                'User-Agent': 'BotIndex/1.0'
            }
