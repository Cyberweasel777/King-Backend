...
exports.TIME_WINDOWS = void 0;
exports.generateCorrelationMatrix = generateCorrelationMatrix;
exports.identifyMarketLeaders = identifyMarketLeaders;
exports.filterByCorrelation = filterByCorrelation;
exports.getTopCorrelatedPairs = getTopCorrelatedPairs;
exports.calculateMatrixStats = calculateMatrixStats;
 ...
 */
function generateCorrelationMatrix(priceSeries, window = '24h') {
    const tokens = priceSeries.map(p => p.token);
    const n = tokens.length;
    // Initialize empty matrix
    const matrix = Array(n).fill(0).map(() => Array(n).fill(0));
    const significance = Array(n).fill(0).map(() => Array(n).fill(0));
    const entries = [];
    // Calculate all pairwise correlations
    const correlationMap = (0, correlation_1.batchCalculateCorrelations)(priceSeries, {
    ...
    for (const anomaly of anomalies) {
        const entry = entries.find(e => (e.tokenA === anomaly.tokenA && e.tokenB === anomaly.tokenB) ||
    ...
    return {
        ...
        window: exports.TIME_WINDOWS[window].name
    };
}
/**
 ...
 */
function detectClusters(matrix, tokens) {
    const n = tokens.length;
    if (n < 2)
        return [];
    ...
    const currentClusters = tokens.map((_, i) => ({ tokens: [i], id: `cluster-${i}` }));
    ...
    while (currentClusters.length > 1) {
        ...
        if (mergeI === -1 || maxAvgCorr < CORR_MODERATE)
            break;
        // Merge clusters
        const mergedTokens = [
 ...
 */
function generateClusterDescription(tokens, avgCorrelation) {
 ...
 */
function detectMatrixAnomalies(matrix, tokens) {
    const anomalies = [];
    const n = tokens.length;
    ...
    for (let i = 0; i < n; i++) {
        for (let j = i + 1; j < n; j++) {
            ...
            const { isAnomaly, severity, direction, zScore } = (0, correlation_1.detectAnomaly)(currentCorr, history, 2.0 // 2 standard deviations
            );
 ...
 */
function identifyMarketLeaders(priceSeries) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        for (let j = 0; j < n; j++) {
            ...
            try {
                const result = (0, correlation_1.calculateCorrelation)(tokenA, tokenB);
                ...
                if (!leadLag.isASecond && leadLag.maxCorrelation > 0.4) {
                    ...
                    numLedTokens++;
                    if (leadLag.causalityStrength === 'strong')
                        strongCausalityCount++;
                    else if (leadLag.causalityStrength === 'moderate')
                        moderateCausalityCount++;
                }
            }
            catch (e) {
        ...
        const confidence = numLedTokens >= (n - 1) * 0.7 ? 'high' :
 ...
 */
function filterByCorrelation(matrix, threshold = 0.5) {
    return matrix.entries.filter(e => Math.abs(e.correlation) >= threshold && e.tokenA !== e.tokenB);
}
/**
 ...
 */
function getTopCorrelatedPairs(matrix, limit = 10, positiveOnly = true) {
...
function calculateMatrixStats(matrix) {
    ...
    for (const entry of entries) {
        ...
        if (absCorr > CORR_STRONG)
            strongCount++;
        else if (absCorr > CORR_MODERATE)
            moderateCount++;
    }
    return {
