...
exports.getBotindexTokenUniverse = getBotindexTokenUniverse;
exports.getCoreBotindexTokenUniverse = getCoreBotindexTokenUniverse;
...
let cachedAt = 0;
function isLikelyBase58Address(value) {
    // Solana addresses are base58 and typically 32-44 chars.
    return /^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(value);
}
async function getBotindexTokenUniverse(targetSize = 30) {
    const now = Date.now();
    ...
    try {
        ...
        const dynamic = profiles
            ...
            .filter((token) => !CORE_TOKEN_UNIVERSE.includes(token));
...
function getCoreBotindexTokenUniverse() {
