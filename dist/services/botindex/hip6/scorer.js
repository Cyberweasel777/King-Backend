...
exports.scoreAuction = scoreAuction;
...
function normalizePercent(value) {
    ...
    if (value <= 10_000)
        return clamp(value / 10_000);
    return 1;
}
function scoreBidConcentration(bids) {
    if (bids.length === 0)
        return 0;
...
function scorePriceStability(clearingEvents) {
    if (clearingEvents.length < 2)
        return 0;
...
function scoreVolumeVelocity(state, clearingEvents) {
...
function scoreParticipantDiversity(bids) {
    if (bids.length === 0)
        return 0;
...
function scoreLiquiditySeedRatio(state) {
    const stateWithSeed = state;
...
function scoreAuction(state, bids, clearingEvents) {
    const factors = {
    ...
    const weightedScore = factors.bidConcentration * WEIGHTS.bidConcentration +
