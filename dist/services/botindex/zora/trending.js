...
function parseTrendingCoin(node) {
    ...
    const address = firstString(record.address, record.coinAddress, record.contractAddress, record.tokenAddress, coin?.address, coin?.coinAddress, coin?.contractAddress);
    ...
    const volume24h = firstNumber(record.volume24h, record.volume24H, record.volume_24h, record.volume24hUsd, record.volume24hUSD, stats?.volume24h, stats?.volume24hUsd, marketStats?.volume24hUsd, metrics?.volume24h, metrics?.volume24hUsd, volume?.h24, volume?.day);
    const priceChange1h = firstNumber(record.priceChange1h, record.price_change_1h, stats?.priceChange1h, marketStats?.priceChange1h, metrics?.priceChange1h, price?.change1h, change?.h1, change?.hour) ?? 0;
    const priceChange24h = firstNumber(record.priceChange24h, record.price_change_24h, stats?.priceChange24h, marketStats?.priceChange24h, metrics?.priceChange24h, price?.change24h, change?.h24, change?.day) ?? 0;
    const holders = firstNumber(record.holders, record.holderCount, record.uniqueHolders, stats?.holders, stats?.holderCount, marketStats?.holderCount, metrics?.holders) ?? 0;
    const tradingFees24h = firstNumber(record.tradingFees24h, record.fees24h, record.creatorFees24h, stats?.tradingFees24h, stats?.creatorFees24h, stats?.fees24h, marketStats?.tradingFees24h, metrics?.tradingFees24h) ?? 0;
...
function collectCandidateNodes(value, depth, output) {
...
function extractCoinsFromPayload(payload) {
    const candidates = [];
...
async function fetchFromZoraGraphQL(limit) {
    const query = `
  ...
  `;
    try {
        ...
        if (!response.ok) {
            logger_1.default.warn({ status: response.status }, 'Zora GraphQL trending query failed');
            return null;
        }
        const payload = (await response.json());
        const payloadRecord = toRecord(payload);
        if (payloadRecord && Array.isArray(payloadRecord.errors) && payloadRecord.errors.length > 0) {
            logger_1.default.warn({ errors: payloadRecord.errors }, 'Zora GraphQL returned errors for trending query');
            return null;
        }
        const coins = extractCoinsFromPayload(payload).slice(0, limit);
        if (coins.length === 0) {
            logger_1.default.warn('Zora GraphQL trending query returned no parsable coins');
            return null;
        }
        return { coins, provider: 'zora_graphql' };
    }
    catch (error) {
        logger_1.default.warn({ err: error }, 'Zora GraphQL unreachable for trending query');
        return null;
    }
}
async function fetchFromZoraRestExplore(limit) {
    for (const baseUrl of ZORA_REST_EXPLORE_URLS) {
        ...
        try {
            ...
            if (!response.ok) {
                logger_1.default.warn({ status: response.status, url: baseUrl }, 'Zora REST explore query failed');
                continue;
            }
            const payload = (await response.json());
            const coins = extractCoinsFromPayload(payload).slice(0, limit);
            if (coins.length === 0) {
                logger_1.default.warn({ url: baseUrl }, 'Zora REST explore returned no parsable coins');
                continue;
            }
            return { coins, provider: 'zora_rest' };
        }
        catch (error) {
            logger_1.default.warn({ err: error, url: baseUrl }, 'Zora REST explore endpoint unreachable');
        }
    }
    return null;
}
function getMockTrendingCoins(limit) {
    const seedCoins = [
        {
...
async function fetchLiveTrendingCoins(limit) {
...
async function getZoraTrendingCoins(limit) {
    ...
    if (data.source === 'mock') {
        logger_1.default.warn('Using mock Zora trending coins as last-resort fallback');
    }
    else {
        logger_1.default.info({ provider: data.provider, count: data.coins.length }, 'Fetched live Zora trending coins');
    }
