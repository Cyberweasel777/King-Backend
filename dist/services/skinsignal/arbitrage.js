...
const analyzer_1 = require("./stats/analyzer");
 ...
 */
async function detectOpportunities(scrapeResults, minNetSpreadPct = 5, useDeepSeek = true) {
    ...
    for (const buy of prices) {
        for (const sell of prices) {
            ...
            if (netSpreadPct < minNetSpreadPct)
                continue;
                ...
                try {
                    const input = {
                        ...
                        prices: prices.map(p => ({
                            ...
                            priceUsd: p.priceUsd,
                            currency: p.currency,
                            feePercent: (types_1.MARKETPLACE_FEES[p.market] ?? 0) * 100,
                        })),
                        timestamp: new Date().toISOString(),
                    };
                    const { data } = await (0, analyzer_1.analyzeSpread)(input);
                    if (data) {
