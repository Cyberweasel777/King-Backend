...
function buildCtaBlock(sourcePath?: string) {
  ...
  return {
    _botindex_intelligence: {
      ...
      pro: {
        url: 'https://api.botindex.dev/api/botindex/keys/register?plan=pro' + sourceParam,
        price: '$9.99/mo',
  ...
  return (_req: Request, res: Response, next: NextFunction) => {
    ...
    res.json = function (body: any) {
      ...
      if (body && typeof body === 'object' && !Array.isArray(body) && !body.error) {
        Object.assign(body, buildCtaBlock(_req.path));
