...
export function createRosterRadarBot(token: string): Telegraf {
  const bot = new Telegraf(token);

  bot.command('start', async (ctx) => {
    await ctx.reply(
      `🏈 *RosterRadar* — DFS + Betting Edge Scanner\n\n` +
      `Commands:\n` +
  ...
  bot.command('lines', async (ctx) => {
    await ctx.reply(
      ...
      `• LAL O/U 231.5 → 233.0\n` +
      `• KC ML -128 → -140\n\n` +
      `_Live data adapter wired; source feed rollout in progress._`,
      { parse_mode: 'Markdown' }
    );
  });
  ...
  bot.command(
    ...
    async (ctx) => {
      await ctx.reply(
        ...
        `_Full lineup payload shipping behind rollout flags._`,
        { parse_mode: 'Markdown' }
      );
    }
  );
...
export default createRosterRadarBot;
