...
function coerceBlockNumber(value: unknown): number | null {
...
export function buildProofInputs(request: AnchorRequest): ProofInputs {
...
function toAnchorMeta(value: unknown): AnchorMeta | null {
...
export class AztecClient {
  private readonly config: AztecConfig;
  private readonly stubAnchors = new Map<string, AnchorMeta>();
  ...
  async anchorProof(request: AnchorRequest): Promise<AnchorResult> {
    ...
    const aztecBlock =
      coerceBlockNumber(row.aztecBlock ?? row.blockNumber ?? row.block) ??
      this.deriveBlock(proofHash);
    const anchorId =
      typeof row.anchorId === 'string' && row.anchorId.trim().length > 0
        ? row.anchorId
  ...
  async verifyProof(proofHash: string): Promise<VerifyResult> {
  ...
  async getAnchors(query: RegistryQuery): Promise<RegistryResult> {
    ...
    const anchors = items
      ...
      .filter((item): item is AnchorMeta => item !== null);
  ...
  private buildSimulatedAnchor(
    proofHash: string,
    timestamp: string
  ): Pick<AnchorResult, 'txHash' | 'aztecBlock' | 'anchorId' | 'timestamp'> {
    return {
    ...
    for (let attempt = 0; attempt <= this.config.maxRetries; attempt += 1) {
      try {
        ...
        logger.warn(
          ...
          'Aztec RPC call failed, retrying'
        );
        await delay(delayMs);
      }
    }
