...
function normalizeHex4Bytes(value: string): string | null {
  const trimmed = value.trim().toLowerCase();
  const normalized = trimmed.startsWith('0x') ? trimmed.slice(2) : trimmed;
...
function coerceHexInteger(value: unknown): number | null {
...
function topicFromSignature(signature: string): string {
...
export function normalizeBaseProofHash(value: string): string | null {
  return normalizeHex32(value);
}

export function buildBaseProofInputs(request: BaseAnchorRequest): BaseProofInputs {
...
function toAnchorMeta(value: unknown, fallbackChainId: number): BaseAnchorMeta | null {
  ...
  const block =
    coerceNonNegativeInteger(row.block) ??
  ...
  const chainId =
    coerceNonNegativeInteger(row.chainId) ??
...
export class BaseClient {
  ...
  private readonly stubAnchors = new Map<string, BaseAnchorMeta>();
  private readonly liveAnchors = new Map<string, BaseAnchorMeta>();
  private readonly contractAddress: string | null;
  private readonly fromAddress: string | null;
  private readonly anchorSelector: string;
  private readonly verifySelector: string;
  private readonly anchoredEventTopic: string;
  ...
  constructor(config: Partial<BaseConfig> = {}) {
    this.config = {
      ...
      chainId:
        config.chainId ?? parseInteger(process.env.BASE_CHAIN_ID, DEFAULT_CHAIN_ID, 1, 1_000_000),
      timeoutMs:
        config.timeoutMs ??
  ...
  async anchorProof(request: BaseAnchorRequest): Promise<BaseAnchorResult> {
  ...
  async verifyProof(proofHash: string): Promise<BaseVerifyResult> {
  ...
  async getAnchors(query: BaseRegistryQuery): Promise<BaseRegistryResult> {
    const limit = Math.max(1, Math.min(200, query.limit));
    const offset = Math.max(0, query.offset);

    if (!this.config.live) {
      let anchors = Array.from(this.stubAnchors.values()).sort(
    ...
    const mergedByHash = new Map<string, BaseAnchorMeta>();
    ...
    let anchors = Array.from(mergedByHash.values()).sort(
      (a, b) => Date.parse(b.timestamp) - Date.parse(a.timestamp)
    );
  ...
  private buildSimulatedAnchor(
    proofHash: string,
    timestamp: string
  ): Pick<BaseAnchorResult, 'txHash' | 'baseBlock' | 'anchorId' | 'timestamp' | 'chainId'> {
    return {
  ...
  private async readChainId(): Promise<number> {
    ...
    if (chainId !== this.config.chainId) {
      logger.warn(
        { configuredChainId: this.config.chainId, rpcChainId: chainId },
        'Configured Base chain ID does not match RPC endpoint chain ID'
      );
    }
  ...
  private async readLatestBlock(): Promise<number> {
  ...
  private async submitAnchorTransaction(
    proofHash: string,
    chainId: number,
    timestamp: string
  ): Promise<string> {
    if (!this.contractAddress || !this.fromAddress) {
      logger.warn(
        { hasContractAddress: Boolean(this.contractAddress), hasFromAddress: Boolean(this.fromAddress) },
        'Base ProofRegistry tx config is missing, using deterministic tx hash placeholder'
      );
    ...
    try {
      ...
      logger.warn({ err: error }, 'Base tx submission failed, using deterministic tx hash placeholder');
      return this.deriveTxHash(proofHash, timestamp);
    }
  }

  private async verifyProofOnChain(proofHash: string): Promise<VerificationOnChain> {
    ...
    try {
      ...
      logger.warn({ err: error, proofHash }, 'Base ProofRegistry eth_call verification failed');
    }
    ...
    try {
      ...
      const logs = rpcResult as Array<Record<string, unknown>>;
      ...
      logger.warn({ err: error, proofHash }, 'Base ProofRegistry log verification failed');
      return { verified: false };
    }
  }

  private async readAnchorsFromChain(): Promise<BaseAnchorMeta[]> {
    if (!this.contractAddress) {
      return [];
    }
    ...
    try {
      ...
      logger.warn({ err: error }, 'Failed to read latest Base block for registry query');
      return [];
    }
    ...
    try {
      ...
      logger.warn({ err: error }, 'Failed to query Base anchor logs from chain');
      return [];
    }
    ...
    const deduped = new Map<string, BaseAnchorMeta>();
  ...
  private async readBlockTimestamp(blockNumber: number): Promise<string> {
    try {
      ...
      logger.warn({ err: error, blockNumber }, 'Failed to read Base block timestamp');
    }

    return new Date().toISOString();
  }

  private decodeEvmBoolean(value: unknown): boolean {
  ...
  private encodeBytes32Call(selector: string, bytes32Hex: string): string {
  ...
  private async callRpcWithRetry(method: string, params: unknown[]): Promise<unknown> {
    ...
    for (let attempt = 0; attempt <= this.config.maxRetries; attempt += 1) {
      try {
        ...
        logger.warn(
          { err: error, method, attempt: attempt + 1, delayMs },
          'Base RPC call failed, retrying'
        );
        await delay(delayMs);
      }
    }
