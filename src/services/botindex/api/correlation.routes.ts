...
import { fetchMultiplePriceSeries, fetchPriceSeries } from '../engine/fetcher';
import {
  submitJob,
  getJobStatus
} from './correlation.cache';
...
// Cache storage
const pairCache = new Map<string, { result: any; expiresAt: number }>();
const matrixCache = new Map<string, { data: any; expiresAt: number }>();

/**
 * GET /correlation/:tokenA/:tokenB
 * Get specific pair correlation
 */
router.get('/correlation/:tokenA/:tokenB', async (req: Request, res: Response) => {
  try {
    const { tokenA, tokenB } = req.params;
    const window = (req.query.window as '1h' | '24h' | '7d' | '30d') || '24h';
 ...
 */
router.get('/correlation/deep/:tokenA/:tokenB', async (req: Request, res: Response) => {
  try {
    const { tokenA, tokenB } = req.params;
    const window = (req.query.window as '1h' | '24h' | '7d' | '30d') || '24h';
    ...
    const localResult = calculateCorrelation(seriesA, seriesB, {
      useReturns: true,
      window: TIME_WINDOWS[window].hours
    });

    // Enhance with DeepSeek analysis
    const deepseekResult = await analyzer.calculateCorrelation({
...
router.get('/correlation/matrix', async (req: Request, res: Response) => {
  try {
    ...
    const tokens = (req.query.tokens as string)?.split(',') || DEFAULT_TOKEN_UNIVERSE;
    ...
    let regimeAnalysis: Record<string, any> = {};
    try {
    ...
    // Build response
    const response: CorrelationMatrixResponse & { regimeAnalysis?: Record<string, any> } = {
 ...
 */
router.get('/correlation/leaders', async (req: Request, res: Response) => {
  try {
    const window = (req.query.window as '1h' | '24h' | '7d' | '30d') || '24h';
    const limit = parseInt(req.query.limit as string) || 10;
    const minLeadScore = parseInt(req.query.minScore as string) || 0;
    ...
    if (cached && cached.expiresAt > Date.now()) {
      const filtered = cached.data.leaders
        .filter((l: any) => l.leadScore >= minLeadScore)
        .slice(0, limit);
    ...
    const response: MarketLeadersResponse = {
 ...
 */
router.get('/correlation/top', async (req: Request, res: Response) => {
  try {
    const window = (req.query.window as '1h' | '24h' | '7d' | '30d') || '24h';
    const limit = parseInt(req.query.limit as string) || 10;
    ...
    // Check cache
    const cacheKey = `matrix:${window}:${tokens.sort().join(',')}`;
    let matrix = matrixCache.get(cacheKey)?.data as CorrelationMatrix | undefined;
    ...
    if (!matrix) {
      ...
      matrix = generateCorrelationMatrix(priceSeries, window);
    }

    const topPairs = getTopCorrelatedPairs(matrix, limit, positiveOnly);
 ...
 */
router.post('/correlation/calculate', async (req: Request, res: Response) => {
  try {
    const { 
      type = 'full_matrix',
      window = '24h',
      tokens = DEFAULT_TOKEN_UNIVERSE,
      priority = 5 
    } = req.body;
    ...
    const response: CalculationTriggerResponse = {
      jobId: job.id,
      status: job.status,
 ...
 */
router.get('/correlation/jobs/:jobId', (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/correlation/stats', async (req: Request, res: Response) => {
  try {
    const window = (req.query.window as '1h' | '24h' | '7d' | '30d') || '24h';
    const tokens = (req.query.tokens as string)?.split(',') || DEFAULT_TOKEN_UNIVERSE;
 ...
 */
router.delete('/correlation/cache', (_req: Request, res: Response) => {
...
function formatPairResponse(result: any): CorrelationPairResponse {
  return {
 ...
 */
async function analyzeMatrixRegimes(priceSeries: any[]): Promise<Record<string, any>> {
  const regimes: Record<string, any> = {};

  for (const series of priceSeries) {
    try {
      const regimeResult = await analyzer.detectRegime({
        prices: series.prices,
        timestamps: series.timestamps || series.prices.map((_: number, i: number) => Date.now() - (series.prices.length - i) * 3600000),
        lookbackDays: 30
