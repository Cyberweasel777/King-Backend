...
export async function getFundingArbOpportunities(): Promise<FundingArbResponse> {
  ...
  try {
    ...
    let binanceFunding = new Map<string, number>();
    let binanceAvailable = false;
    try {
      binanceFunding = await fetchBinanceFundingMap();
      binanceAvailable = true;
    } catch (binanceErr) {
      logger.warn({ err: binanceErr }, 'Binance funding fetch failed (likely geo-block); falling back to Hyperliquid-only mode');
    }
