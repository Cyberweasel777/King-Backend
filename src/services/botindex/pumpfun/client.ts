...
import { scorePumpfunRug } from './scorer';
...
let graduatingCache: CacheEntry<PumpfunToken[]> | null = null;
let graduatedCache: CacheEntry<PumpfunGraduation[]> | null = null;
const rugScoreCache = new Map<string, CacheEntry<PumpfunRugScore>>();
...
const STUB_TOKENS: PumpfunToken[] = [
  {
...
const STUB_GRADUATIONS: PumpfunGraduation[] = [
  {
    token: {
      ...STUB_TOKENS[0],
      isGraduated: true,
...
export async function getGraduatingTokens(): Promise<PumpfunToken[]> {
  if (isFresh(graduatingCache)) return graduatingCache.data;

  if (!PUMPFUN_LIVE) {
    logger.info('[pumpfun] stub mode — returning mock graduating tokens');
  ...
  logger.warn('[pumpfun] live mode not yet implemented — returning empty');
  return [];
}

export async function getRecentGraduations(limit = 10): Promise<PumpfunGraduation[]> {
  if (isFresh(graduatedCache)) return graduatedCache.data.slice(0, limit);

  if (!PUMPFUN_LIVE) {
    logger.info('[pumpfun] stub mode — returning mock graduations');
...
export async function getRugScore(mintAddress: string): Promise<PumpfunRugScore> {
  const cached = rugScoreCache.get(mintAddress);
  if (isFresh(cached)) return cached.data;

  if (!PUMPFUN_LIVE) {
    logger.info(`[pumpfun] stub mode — returning mock rug score for ${mintAddress}`);
    const stubGrad = STUB_GRADUATIONS.find(g => g.token.mintAddress === mintAddress);
    const score = scorePumpfunRug({
      mintAddress,
  ...
  logger.warn(`[pumpfun] live mode not yet implemented for ${mintAddress}`);
  return scorePumpfunRug({
    mintAddress,
