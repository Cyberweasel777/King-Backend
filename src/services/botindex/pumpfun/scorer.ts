...
function holderConcentrationSafety(topHolderConcentration: number): number {
...
function scoreHolderConcentrationRisk(topHolderConcentration: number): number {
  const safety = holderConcentrationSafety(topHolderConcentration);
  return clamp(100 - safety);
}

function scoreDevWalletActivityRisk(inputs: PumpfunRiskInputs): number {
...
function scoreLiquidityLockedRisk(inputs: PumpfunRiskInputs): number {
  if (!inputs.liquidityLocked) {
    return 95;
  }

  const lockDays = clamp(inputs.liquidityLockDays ?? 0, 0, 3650);
  if (lockDays >= 365) return 5;
  if (lockDays >= 180) return 15;
  if (lockDays >= 90) return 30;
  if (lockDays >= 30) return 50;
  if (lockDays > 0) return 65;
  return 75;
}

function scoreSocialSignalsRisk(inputs: PumpfunRiskInputs): number {
  const flags = Math.max(0, Math.floor(inputs.socialRiskFlags ?? 0));
  if (flags >= 4) return 100;
  if (flags === 3) return 85;
  if (flags === 2) return 70;
  if (flags === 1) return 55;
...
function scoreBuySellAnomalyRisk(buySellRatio: number): number {
...
function scoreTradingPatternRisk(inputs: PumpfunRiskInputs): number {
...
export function scorePumpfunRug(inputs: PumpfunRiskInputs): PumpfunRugScore {
  const factors = {
  ...
  const weighted =
    factors.holderConcentration * WEIGHTS.holderConcentration +
