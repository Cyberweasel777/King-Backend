...
import { buildProvenanceReport } from './provenance';
...
export interface AlertTelemetry {
  ...
  holderDispersionScore?: number; // lower = more concentrated
}
...
export function evaluateAlerts(t: AlertTelemetry): TriggeredAlert[] {
...
export function buildDailyDigest(tokens: TrendingToken[], previousScores: Map<string, number>): DigestItem[] {
  return tokens.map((t) => {
    const report = buildProvenanceReport(t.token);
