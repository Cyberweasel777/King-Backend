...
export type ProvenanceFactorName =
...
const WEIGHTS: Record<ProvenanceFactorName, number> = {
...
function ageHours(token: TokenData): number {
...
export function buildProvenanceReport(token: TokenData): ProvenanceReport {
  ...
  const origin = clamp(
    (token.metadata?.creator ? 80 : 35) +
      (warnings.includes('missing_liquidity') ? -20 : 0) +
      (warnings.includes('missing_volume24h') ? -10 : 0)
  );

  const liqGenesis = clamp(
    (liquidity > 1_000_000 ? 85 : liquidity > 250_000 ? 65 : liquidity > 50_000 ? 45 : 25) +
      (hours < 24 ? -20 : 0)
  );
  ...
  const tokenomics = clamp(
    (token.metadata?.mintAuthority ? 40 : 70) +
      (token.metadata?.freezeAuthority ? -20 : 0)
  );

  const concordance = clamp(
    85 +
      (warnings.includes('missing_priceChange24h') ? -20 : 0) +
      (warnings.includes('missing_txns24h') ? -15 : 0)
  );

  const microstructure = clamp(
    (volume > 5_000_000 ? 80 : volume > 1_000_000 ? 70 : volume > 200_000 ? 55 : 40) +
  ...
  if (hours < 24 && liquidity < 250_000) whyFlagged.push('Liquidity fragility: LP <24h and liquidity under $250k.');
  if (absMove > 80) whyFlagged.push('Extreme 24h move indicates unstable microstructure.');
  if (warnings.length >= 2) whyFlagged.push('Data quality gaps detected across source fields.');
  if (token.metadata?.freezeAuthority) whyFlagged.push('Freeze authority present — elevated centralization risk.');
