...
import { sentimentCache } from '../shared/cache';
...
export class TwitterScraper extends BaseScraper {
  ...
  private trackedTokens: Map<string, { lastChecked: string; mentionCount: number }> = new Map();
  ...
  async searchTokenMentions(
    ...
    const cached = sentimentCache.get<SentimentData>(cacheKey);
    if (cached) {
    ...
    try {
      ...
      const response = await this.withRetry(async () => {
        ...
        const res = await this.fetchWithTimeout(
          `${this.baseUrl}/tweets/search/recent?query=${encodeURIComponent(query)}&` +
      ...
      const engagement = tweets.reduce((sum, t) => 
        sum + (t.publicMetrics?.likeCount || 0) + (t.publicMetrics?.retweetCount || 0), 0
      );
      ...
      sentimentCache.set(cacheKey, result, 300000); // 5 minute cache
   ...
   */
  async monitorTokens(tokens: string[]): Promise<SentimentData[]> {
   ...
   */
  async scrape(): Promise<{ sentiments: SentimentData[] }> {
  ...
  // Helper methods
  private calculateSentiment(tweets: Tweet[]): { score: number; trending: boolean } {
    if (tweets.length === 0) return { score: 0, trending: false };
    ...
    if (total === 0) return { score: 0, trending: totalEngagement > 1000 };
  ...
  private getMockSentiment(token: string): SentimentData {
