...
export class SteamMarketScraper extends BaseScraper {
  private readonly appId = 730;  // CS2
  private readonly currency = 1; // USD
  ...
  async scrape(skinName: string): Promise<ScrapeResult> {
    ...
    try {
      const data = await this.withRetry(() =>
        this.request<SteamPriceOverview>({
          method: 'GET',
...
export const steamScraper = new SteamMarketScraper();
